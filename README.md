...
A pixel is no longer a pixel. There are effectively two kinds of pixels in the world: hardware pixels and reference pixels. Hardware pixels are the old, familiar concept of the pixel: which are the actual points of light on the screen of a given device. A 1024 × 768 monitor built in 2002 had precisely 1024 pixels/points of light across, and 768 points on the vertical. But retina displays, as Apple called them, or pixel-doubled and -tripled displays lead [the W3C to develop the concept of a reference pixel](http://www.w3.org/TR/css-values/#reference-pixel): “the visual angle of one pixel on a device with a pixel density of 96dpi and a distance from the reader of an arm’s length.”
